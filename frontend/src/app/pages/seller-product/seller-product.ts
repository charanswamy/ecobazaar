...
export class SellerProduct implements OnInit, OnDestroy {
  ...
  // --------------------------------------------------
  // ✔️ ADDED: Role-based page class for seller pages
  // --------------------------------------------------
  ngOnInit() {
    // Remove previous page roles
    document.body.classList.remove('user-page', 'admin-page', 'seller-page');

    // Add seller page class (you will style this in global.scss)
  ...
  // --------------------------------------------------
  // ✔️ CLEANUP when leaving seller-product page
  // --------------------------------------------------
  ngOnDestroy(): void {
    document.body.classList.remove('seller-page');
  }

  t = (k: 'name' | 'details' | 'price' | 'carbonImpact' | 'ecoCertified') =>
    this.form.controls[k];
    ...
    const sendPayload = (imageUrl?: string | null) => {
      ...
      obs.subscribe({
        next: () => {
          ...
          this.toastr.success(
            this.editing ? 'Product updated' : 'Product created'
          );
        ...
        error: (err) => {
          ...
          this.uploading = false;
          const msg =
            err?.error?.message || err?.message || 'Failed to save product';
          this.error = msg;
    ...
    if (this.selectedFile) {
      this.cloud.uploadFile(this.selectedFile).subscribe({
        ...
        error: (err) => {
          ...
          this.uploading = false;
          const msg =
            err?.error?.message || err?.message || 'Image upload failed';
          this.error = msg;
          this.toastr.error(msg);
        }
      });
